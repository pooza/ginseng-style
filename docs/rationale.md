...
配っている途中で、同じ cop（`Minitest/ReturnInTestMethod`）が別のリポジトリのテストを壊していたことが分かった。`return if Environment.ci?` が **`skip if ...` へ自動修正されていた**（2025-01-02）。

- test-unit に `skip` は無い。CI のテストは `NameError` で落ち続けていた
- ⚠⚠ **1 年半以上、誰も気づいていなかった**
- ⚠ **lint は緑。** 「テストが赤い CI」を誰も見ていなかったことと合わせて初めて成立する
- 横断で `test/` 配下の `skip` を grep して、他に無いことを確認した

⚠⚠ **1 リポジトリだけを見ていると、これは「そのリポジトリのテストが壊れている」に見える。** 正本を持って初めて「cop の自動修正が原因で、同じ書き方をした全員が踏みうる」と分かる。
