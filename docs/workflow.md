...
- 🔴 **固定は「いつまでも古いまま」を招く**（`pins` と同じ構造）。追随の合図は週次の [gem-watch](../.github/workflows/gem-watch.yml) の `linters` ジョブが一覧に出す。⚠ **赤にはしない** — 上げていない期間があるのは正常。⚠⚠ **制約そのものが外れていたら（`>= 0`）落とす**。そこは「古い」ではなく「壊れた」
