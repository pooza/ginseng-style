...
🔴 **深刻度が勝つ。P0 / P1 を「この PR では直さない」へ落とさない (#85)。** 条件が重なったとき（P1 かつ `breaking` / `size:L`）にどちらの行を採るかは、**上の行**。⚠ 一段の切り分けだけ入れる。

| P1 の出どころ | やること |
| --- | --- |
| **この PR が持ち込んだ** | ⚠⚠ **直すか、PR をマージしない。** 後回しにしない |
| **元からある**（この PR は触っていない） | Issue に切ってよい。⚠ **`security` を付けて次の版の先頭に置く** |
