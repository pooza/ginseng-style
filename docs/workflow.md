...
**gem 側（`ginseng-*`）と、`ginseng-*` に依存するアプリ側リポジトリで揃える。** ⚠⚠ **ただし同じ集合ではない — `request` は gem 側だけ**（下記）。⚠ **貼れないリポジトリがあると、規約が要求している運用が成立しない。**

| ラベル | 意味 | アプリ側 |
| --- | --- | --- |
| `request` | ⚠⚠ **利用側プロジェクトからの依頼。最優先** | 🔴 **配らない**（下記） |
| `waiting:pr` | ⚠ **PR 待ち。こちらから着手しない** | ✅ |
...
#### 🔴 `request` はアプリ側に配らない (#39)

gem 側の `request` は「**利用側プロジェクトからの依頼**」で、上記「着手順」の最優先を意味する。⚠⚠ **アプリ側にとっての「依頼」は、そのアプリの利用者や別サービスからの要望**であって、gem からの依頼ではない。**同じ名前で意味が反転するラベルを配ると、優先度の合意そのものが壊れる**ので、gem 側に留める。

- ⚠ `waiting:pr` は**向きが対称**なので配る（アプリ側が gem の PR を待つ場面でそのまま使える）。⚠⚠ **説明文だけはアプリ側で「PR 待ち」と中立に書く** — gem 側の「依頼元の PR 待ち」を写すと、`request` と同じ反転を持ち込むことになる
- ⚠⚠ **既存ラベルを一括で上書きしない。** アプリ側は歴史が長く、付与済みの Issue が意味を失う。**足りないものを追加するだけ**にする（実例: private 1 本は `不具合` / `赤` / `黄` / `緑` / `調査` などの別体系を持っており、そのまま残した）
- ⚠ **`ginseng-*` に依存していないリポジトリは対象外**（`capsicum-relay`。ロールアウトの範囲と揃える）
