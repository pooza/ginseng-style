...
🔴 **正本はスキルに移した (#104)** — [plugins/ginseng/skills/codex-review/SKILL.md](../plugins/ginseng/skills/codex-review/SKILL.md)。採否の判断・👎 の規律・「指摘なし」の 3 通り・取り残しの走査・待機ループの組み方は、すべてそちらにある。

- **ここに残すのは判定の規則だけ**: PR には Codex（`chatgpt-codex-connector[bot]`）のレビューが**遅れて**届く。⚠⚠ **返信と 👍 / 👎 の両方を付けて完了とし、片方だけのものは未処理として扱う**
- ⚠ **スキルとして `/ginseng:codex-review` で呼ぶには、プラグインを入れる必要がある**（[skills.md](skills.md)）。⚠⚠ **入れていない環境からも読めるように、正本は普通のファイルとして置いてある** — リンクを開けば全文がある
- ⚠ **二重に持たない。** 手順を足すときはスキルの側だけを直す
