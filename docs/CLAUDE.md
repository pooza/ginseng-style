...
- **`ginseng-*` 共通の手順をスキルとして配る** (#104)。⚠⚠ **正本はスキル側（`plugins/ginseng/skills/`）で、`docs/` はポインタ** — 手順を足すときはスキルだけを直す。⚠ 入れ方と版の扱いは [skills.md](skills.md)
...
5. 問題なければ残りへ配る。1 リポジトリ 1 PR、`rake lint` が緑になったところまで。⚠ **参照 3 経路（`Gemfile` の `ref:` / `test.yml` の `ruby-check@` / `release.yml` の `release-tag@`）を同じタグの SHA に揃える**。⚠⚠ **`--show-cops` と `--list-target-files` の差分を確認する**（[README](../README.md) の `Include` / `Exclude` の置換）。⚠ **既定は「どちらもゼロ」だが、`Include` や cop を意図して変えた版ではゼロにならないのが正しい** — 🔴 **その版では「増えるべきパスが増えたこと」が期待値で、差分ゼロのほうが失敗の合図**（下記）
...
⚠ **rubocop 本体の版を上げるのもこの手順。** `ginseng-style.gemspec` の悲観固定（`~> x.y.0`。#55）を上げるのは**配布物を触る変更**なので、⚠⚠ **バンプが要る／直列化の対象**。⚠ 新しい cop はここでまとめて入るので、手順 5 の `--show-cops` は**差分ゼロにならないのが正しい**。追随の合図は週次 gem-watch の `linters`（[workflow.md](workflow.md) の「上限の理由は事故だけではない」）。
